...
class MainActivity : AppCompatActivity() {
    ...
    private var currentSourceLabel: String = "Nothing loaded"
    ...
    private val openAudioLauncher =
        registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
            if (uri != null) {
                ...
                loadMedia(uri, "Audio file", autoPlay = true)
            } else {
    ...
    private val openVideoLauncher =
        registerForActivityResult(ActivityResultContracts.OpenDocument()) { uri ->
            if (uri != null) {
                ...
                loadMedia(uri, "Video file", autoPlay = true)
            } else {
        ...
        player?.addListener(object : Player.Listener {
            override fun onPlaybackStateChanged(playbackState: Int) {
                when (playbackState) {
                    ...
                    Player.STATE_BUFFERING -> updateNowPlaying("$currentSourceLabel • Buffering...")
                    Player.STATE_READY -> {
                        if (!player!!.isPlaying) {
                            if (playbackPosition == 0L) {
                                updateNowPlaying("$currentSourceLabel • Ready")
                            } else {
                                updateNowPlaying("$currentSourceLabel • Paused")
                            }
                        }
                    }
                    Player.STATE_ENDED -> updateNowPlaying("$currentSourceLabel • Finished")
                }
            }
            ...
            override fun onIsPlayingChanged(isPlaying: Boolean) {
                updateNowPlaying(
                    if (isPlaying) {
                        "$currentSourceLabel • Playing"
                    } else {
                        if (currentUri == null) "Nothing loaded" else "$currentSourceLabel • Paused"
                    }
                )
            }
        })
    ...
    private fun loadMedia(uri: Uri, sourceLabel: String, autoPlay: Boolean) {
        ...
        updateNowPlaying(
            if (autoPlay) "$sourceLabel • Playing" else "$sourceLabel • Ready"
        )
        showToast("$sourceLabel loaded")
    }
    ...
    private fun loadFromUrl() {
        ...
        loadMedia(uri, "Network stream", autoPlay = true)
    ...
    private fun playCurrent() {
        ...
        player?.play()
        playWhenReady = true
        updateNowPlaying("$currentSourceLabel • Playing")
    }
    ...
    private fun stopCurrent() {
        ...
        playWhenReady = false
        playbackPosition = 0L
        updateNowPlaying("$currentSourceLabel • Stopped")
    }
