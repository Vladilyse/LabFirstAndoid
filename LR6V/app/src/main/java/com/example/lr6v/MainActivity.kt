...
class MainActivity : AppCompatActivity() {
    ...
    private fun setupListeners() {
        ...
        btnEnableAlerts.setOnClickListener {
            enableAlerts()
        }

        btnDisableAlerts.setOnClickListener {
            disableAlerts()
        }
    ...
    private fun enableAlerts() {
        ...
        val workRequest = PeriodicWorkRequestBuilder<WeatherAlertWorker>(
            ...
            workDataOf(WeatherAlertWorker.KEY_CITY to city)
        ).build()
        ...
        if (canPostNotifications()) {
            NotificationHelper.showWeatherNotification(
                context = this,
                title = "Weather Alert",
                message = "Test notification: alerts for $city have been enabled."
            )
        }
    }
    ...
    private fun buildAlertMessage(weather: WeatherData): String? {
        return when {
            weather.temperature <= 0 -> {
                "Frost warning. Temperature is ${"%.1f".format(weather.temperature)} °C."
            }
            weather.temperature >= 30 -> {
                "Heat warning. Temperature is ${"%.1f".format(weather.temperature)} °C."
            }
            weather.windSpeed >= 50 -> {
                "Strong wind warning. Wind speed is ${"%.1f".format(weather.windSpeed)} km/h."
            }
            WeatherCodeMapper.isRainOrSnow(weather.weatherCode) -> {
                "Precipitation alert. Current condition: ${WeatherCodeMapper.toText(weather.weatherCode)}."
            }
            else -> null
        }
    }
    ...
    private fun showLoading(isLoading: Boolean) {
        ...
        btnEnableAlerts.isEnabled = !isLoading
        btnDisableAlerts.isEnabled = !isLoading
        btnTestNotification.isEnabled = !isLoading
    }
